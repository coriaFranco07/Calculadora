...
function getCategoriaValue(categoria) {
  return toNumber(categoria?.basico_mensual ?? categoria?.valor ?? categoria?.unit_price ?? categoria?.sueldo_mensual ?? 0);
}
...
function calculatePayroll(payload, form) {
  ...
  const antiguedadPct = toNumber(antiguedadRule?.porcentaje_por_anio ?? antiguedadRule?.valor, 1);
  ...
  const zonaPct = toNumber(zonaRule?.porcentaje ?? zonaRule?.valor, 0);
      ...
      <div class="dynamic-calculator-body">
        ...
        <div class="dynamic-calculator-checks">
          <label class="dynamic-calculator-check">
            ...
            <span><strong>Aplicar antigüedad</strong><small>${escapeHtml(antiguedadRule ? `${antiguedadRule.porcentaje_por_anio || antiguedadRule.valor || 1}% por año` : "Sin regla detectada")}</small></span>
          </label>
          <label class="dynamic-calculator-check">
            <input type="checkbox" data-calc-apply-zona>
            <span><strong>Aplicar zona / región</strong><small>${escapeHtml(zonaRule ? `${zonaRule.porcentaje || zonaRule.valor || 0}% según convenio` : "Sin zona detectada")}</small></span>
          </label>
        </div>
