...
LOGO_URL = "https://media.licdn.com/dms/image/v2/D4D0BAQGibhoMeCL-mg/company-logo_200_200/company-logo_200_200/0/1667320508791/e_sueldos_logo?e=2147483647&v=beta&t=6BpHD4ZVbBNymqB5aEkJNEXyJ-0qGr2At_F6mEePRAw"
...
function seniorityCalc(years, proportionalBase) {{
  const rule = DATA.reglas_liquidacion?.antiguedad || {{}};
  const parsedYears = Math.max(0, Math.floor(num(years)));
  const scale = Array.isArray(rule.escala) ? rule.escala : [];
  const row = scale.find(x => num(x.anio) === parsedYears) || [...scale].reverse().find(x => num(x.anio) <= parsedYears);
  let pct = num(row?.porcentaje);
  if (!pct && parsedYears) pct = parsedYears * (num(rule.porcentaje_por_anio) || 1);
  const maxPct = scale.length ? Math.max(...scale.map(x => num(x.porcentaje))) : 0;
  if (maxPct) pct = Math.min(pct, maxPct);
  const ruleBase = num(rule.base_monto ?? rule.base ?? rule.seniority_base);
  const calcBase = ruleBase || proportionalBase;
  return {{years: parsedYears, pct, base: calcBase, amount: calcBase * pct / 100}};
}}
...
function init() {{
  ...
  get('p1cat').innerHTML = cats.length ? cats.map((c,i)=>`<option value="${{i}}">${{c.nombre || 'Categoria '+(i+1)}}</option>`).join('') : '<option value="0">Sin categorias detectadas</option>';
  get('scaleRows').innerHTML = cats.length ? cats.map(c=>{{ const a=categoryAmount(c); return `<tr><td>${{c.nombre||'-'}}</td><td>${{c.tipo||'-'}}</td><td class="num">${{a.monthly?fmt(a.monthly):'-'}}</td><td class="num">${{a.hourly?fmt(a.hourly):'-'}}</td><td>${{c.fuente_textual||'-'}}</td></tr>`; }}).join('') : '<tr><td colspan="5">No se detectaron categorias.</td></tr>';
...
function calcular() {{
  ...
  const antYears=num(get('p3ant').value), add=num(get('p3add').value), manualDesc=num(get('p3desc').value), sindPct=num(get('p3sind').value);
  ...
  const antigCalc = seniorityCalc(antYears, base); const antig = antigCalc.amount; const extras = hour*h50*1.5 + hour*h100*2; const bruto = base + antig + extras + add;
  ...
  get('res-recibo').innerHTML = `<div class="rec-hdr-box"><div class="rec-emp">Recibo estimado</div><div class="rec-meta"><span>${{get('p1n').value || 'Trabajador'}}</span><span>${{get('p1c').value || 'CUIL no informado'}}</span><span>${{cat.nombre || 'Categoria'}}</span></div></div><div class="rec-cols"><div class="rec-block"><h4>Haberes</h4><div class="rec-row"><span class="rc">Basico proporcional</span><span class="rm pos">${{fmt(base)}}</span></div><div class="rec-row"><span class="rc">Antiguedad (${{antigCalc.years}} anos  -  ${{antigCalc.pct}}%)</span><span class="rm pos">${{fmt(antig)}}</span></div><div class="rec-row"><span class="rc">Horas extra</span><span class="rm pos">${{fmt(extras)}}</span></div><div class="rec-row"><span class="rc">Adicional manual</span><span class="rm pos">${{fmt(add)}}</span></div></div><div class="rec-block"><h4>Descuentos</h4><div class="rec-row"><span class="rc">Jubilacion 11%</span><span class="rm neg">${{fmt(jubilacion)}}</span></div><div class="rec-row"><span class="rc">Obra social 3%</span><span class="rm neg">${{fmt(obra)}}</span></div><div class="rec-row"><span class="rc">Ley 19.032 3%</span><span class="rm neg">${{fmt(ley)}}</span></div><div class="rec-row"><span class="rc">Sindicato</span><span class="rm neg">${{fmt(sindicato)}}</span></div></div></div><div class="tot-card"><div class="tot-row"><span>Total haberes</span><span class="tv">${{fmt(bruto)}}</span></div><div class="tot-row"><span>Total descuentos</span><span class="tv">${{fmt(descuentos)}}</span></div><div class="tot-row main"><span>Total bolsillo</span><span class="tv">${{fmt(neto)}}</span></div></div>`;
  get('res-calc').innerHTML = `<p class="hint">Base: ${{fmt(base)}}. Antiguedad: ${{antigCalc.years}} anos = ${{antigCalc.pct}}% sobre ${{fmt(antigCalc.base)}} = ${{fmt(antig)}}. Extras: ${{fmt(extras)}}. Descuentos: ${{fmt(descuentos)}}.</p>`;
  const alerts = [...(DATA.alertas||[]), ...(DATA.pendientes_revision||[])];
  get('res-audit').innerHTML = alerts.length ? alerts.map(x=>`<div class="callout warn"><span>[!]</span><span>${{x}}</span></div>`).join('') : '<div class="callout info"><span>[i]</span><span>Sin alertas informadas.</span></div>';
  ip(4); toast('Liquidacion calculada');
}}
init();
</script>
<script type="module" src="/js/cct-chatbot.js"></script>
</body>
</html>'''
